...
class RayCreationFromPixelBaseTests(object):
    ...
    def test_ray_through_center_dist_to_camPointTo(self):
        ...
        ray = self.scene.create_ray(camera.imageHeight/2., camera.imageWidth/2.)
        ...
        nptest.assert_array_almost_equal(point_at_dist, camera.pointTo)
    ...
    def test_angle_between_left_right_extreme_rays(self):
        ...
        #print(est_angle_deg)
        # check fov
        nptest.assert_approx_equal(est_angle_deg, camera.aspect * camera.fov, significant=5)
...
class TestRayCreationFromPixelCamOnPosZ_101x101(RayCreationFromPixelBaseTests, unittest.TestCase):
    ...
    def setUp(self):
        ...
        camera = Camera({'from':np.array([0.,0.,4.]),
                         ...
                         'up':np.array([0.,1.,0.]),
                         'fov':45,
                         'width':101, 'height':101}) # choose very high dimension to avoid rounding error
        render = Render({'camera':camera})
        scene.render = render
...
# TODO: Test for get_nearest_object_intersection()
